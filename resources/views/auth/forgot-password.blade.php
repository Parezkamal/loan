...
<body class="bg-gray-100 flex items-center justify-center min-h-screen">
    <div class="bg-white shadow-md rounded-lg p-8 w-full sm:w-96 max-w-md">
        ...
        <form method="POST" action="{{ route('password.email') }}">
            ...
            @if ($errors->has('email'))
                <div class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rouned relative" role="alert">
                    <ul>
                        @if ($errors->has('email'))
            ...
            <button type="submit" class="w-full bg-white border border-blue-500 text-blue-500 py-2 rounded-md hover:bg-blue-500 hover:text-white focus:outline-none focus:ring focus:ring-blue-200 focus:ring-opacity-50">Send Reset Link</button>
        </form>
